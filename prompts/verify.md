---
description: Run all verification commands for changed files and report results
---
Detect changed files using git diff. Run `pre-commit run --files` on all changed Python files. Then identify affected services and run `pytest` for each. Report what passed and what failed.

Arguments: $@

Special case: if the arguments explicitly ask for a pi-web capability check, do that first with `web_search`, `fetch_content`, or `code_search`, then return the result without running the full verification workflow.

If `src/custom_lib/py/bliv_lib/` changed, find ALL consuming services and test them too.

Use the `verify_changes` tool if available. Otherwise run the commands manually.

Default to local verification commands. If the user explicitly asks for external docs, framework behavior, or a pi-web capability check, use `web_search`, `fetch_content`, or `code_search` to support the verification findings.
