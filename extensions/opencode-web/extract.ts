...
// Claude Code pattern: identify honestly for robots.txt compliance
const CLAUDE_USER_AGENT = "Claude-User/1.0 (pi-web-access; +https://github.com/badlogic/pi-mono)";
...
// Claude Code pattern: save binary responses to disk with correct extension
const BINARY_SAVE_TYPES: Record<string, string> = {
...
// Claude Code pattern: proxy support via environment variables
// Note: Node's native fetch doesn't support proxy agents directly.
// Set HTTPS_PROXY / HTTP_PROXY in the environment and use an undici
// ProxyAgent (or node-fetch + https-proxy-agent) to route requests.
function getProxyAgent(): string | undefined {
	return (
...
async function extractViaHttp(
	...
	try {
		const acceptHeader = "text/html,application/xhtml+xml,application/xml;q=0.9,text/markdown;q=0.8,text/plain;q=0.7,*/*;q=0.5";
		const fetchOptions = {
		...
		// Cloudflare bot detection retry (from opencode)
		let response = initialRes;
		if (initialRes.status === 403) {
			const cfMitigated = initialRes.headers.get("cf-mitigated");
			if (cfMitigated === "challenge") {
				response = await fetch(url, {
					...fetchOptions,
					headers: { ...fetchOptions.headers, "User-Agent": "pi-web-access" },
				});
			}
		}

		// Browser UA fallback for sites that block non-browser agents
		if (response.status === 403 || response.status === 406) {
			response = await fetch(url, {
				...fetchOptions,
				headers: { ...fetchOptions.headers, "User-Agent": BROWSER_USER_AGENT },
			});
		}
		...
		const binaryExt = BINARY_SAVE_TYPES[mimeType];
		if (binaryExt) {
			try {
				const buffer = await response.arrayBuffer();
				const id = Date.now().toString(36) + Math.random().toString(36).slice(2);
				const tmpPath = `/tmp/pi-fetch-${id}${binaryExt}`;
				await writeFile(tmpPath, Buffer.from(buffer));
				activityMonitor.logComplete(activityId, response.status);
				return {
					url,
					title: mimeType,
					content: `Binary file saved to: ${tmpPath}`,
					error: null,
				};
			} catch (err) {
				const message = err instanceof Error ? err.message : String(err);
				activityMonitor.logError(activityId, message);
				return { url, title: "", content: "", error: `Failed to save binary file: ${message}` };
			}
		}
