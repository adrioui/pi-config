...
export async function search(query: string, options: FullSearchOptions = {}): Promise<SearchResponse> {
	...
	if (provider === "exa") {
		try {
			...
			// If explicitly requested but fails with API error, try other providers
			if (isPerplexityAvailable()) {
				console.error(`[opencode-web] Exa failed (${msg}), falling back to Perplexity`);
				return searchWithPerplexity(query, options);
			}
			const geminiResult = await searchWithGeminiApi(query, options) ?? await searchWithGeminiWeb(query, options);
			if (geminiResult) {
				console.error(`[opencode-web] Exa failed (${msg}), fell back to Gemini`);
				return geminiResult;
			}
	...
	if (isExaAvailable()) {
		try {
			...
			// Fall through to next provider on API errors (out of credit, rate limit, etc.)
			const msg = err instanceof Error ? err.message : String(err);
			if (!msg.includes("timed out") && !msg.includes("AbortError")) {
				// Log but continue to next provider
				console.error(`[opencode-web] Exa search failed, trying next provider: ${msg}`);
			} else {
				throw err; // Re-throw timeouts and aborts
			}
		}
	}
