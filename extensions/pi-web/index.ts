...
import { search, type SearchProvider } from "./search-router.js";
...
import { isCodeSearchAvailable, searchCode, type CodeSearchOptions } from "./code-search.js";
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		name: "fetch_content",
		label: "Fetch Content",
		description: "Fetch URL(s) and extract readable content as markdown. Supports YouTube video transcripts (with thumbnail), GitHub repository contents, and local video files (with frame thumbnail). Video frames can be extracted via timestamp/range or sampled across the entire video with frames alone. Falls back to Anthropic web_fetch and Jina Reader for pages that block bots or fail Readability extraction. For YouTube and video files: ALWAYS pass the user's specific question via the prompt parameter — this directs the AI to focus on that aspect of the video, producing much better results than a generic extraction. Content is always stored and can be retrieved with get_search_content.",
		parameters: Type.Object({
			...
			model: Type.Optional(Type.String({
				description: "Override the model for video/YouTube analysis. Defaults to claude-haiku-4-5.",
			})),
		}),
	...
	pi.registerTool({
		name: "code_search",
		label: "Code Search",
		description: "Search for code examples, API documentation, and library references using Exa AI. Returns relevant code context for programming queries. Use when you need to look up API usage patterns, library documentation, or code examples from the web.",
		promptSnippet: "code_search: Search code examples and API docs via Exa AI",
		promptGuidelines: [
			"Use code_search for programming API lookups, library reference, SDK documentation",
			"Specify framework/library names and version when possible",
			"Adjust tokensNum based on scope: 1000-5000 for focused, 10000-50000 for comprehensive",
		],
		parameters: Type.Object({
			query: Type.String({
				description: "Search query for code context. Example: 'React useState hook examples', 'Express.js middleware patterns', 'Python pandas dataframe filtering'",
			}),
			tokensNum: Type.Optional(
				Type.Number({
					description: "Number of tokens to return (1000-50000, default 5000). Use lower values for focused queries, higher for comprehensive docs.",
					minimum: 1000,
					maximum: 50000,
					default: 5000,
				}),
		...
		async execute(_toolCallId, params, signal, onUpdate) {
			if (!isCodeSearchAvailable()) {
				throw new Error(
					"Code search requires Exa AI. Set EXA_API_KEY environment variable or exaApiKey in ~/.pi/web-search.json"
				);
			}

			onUpdate?.({
				content: [{ type: "text", text: `Searching code: ${params.query}` }],
				details: { phase: "searching" },
			});

			const result = await searchCode(params.query, {
				tokensNum: params.tokensNum,
				signal: signal ?? undefined,
			});

			return {
				content: [{ type: "text", text: result.content || "No code context found. Try a different query or be more specific about the library/API." }],
				details: {
					query: result.query,
		...
		renderCall(args: Record<string, unknown>, theme: any) {
			...
			const tokens = Number(args.tokensNum || 5000);
			const lines = [
				theme.fg("toolTitle", "code search ") + theme.fg("accent", `"${query}"`),
			];
			if (tokens !== 5000) {
